        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-12 flex items-center justify-between h-20">
          ...
          <div style={{ display: 'flex', alignItems: 'center' }}>
            <img
              ...
              alt="Logo Catering"
              className="h-18 object-cover object-center bg-[#e5fbff]"
            />
            <a
              href="#"
              onClick={(e) => { e.preventDefault(); window.scrollTo({ top: 0, behavior: 'smooth' }) }}
              className={`font-serif text-2xl font-semibold tracking-widest transition-colors duration-300 ${scrolled ? 'text-foreground' : 'text-white'
                }`}
              style={{ display: 'block', margin: '0 10px' }}
            >
              <div>CÔNG TY TNHH <span className={`italic ${scrolled ? 'text-[#84B4C8]' : 'text-[#6de2ff]'}`}>KỲ THƯ</span></div>
              <div style={{ fontSize: '14px' }}>Hẻm 36, DT 786, Tổ 17, Thanh Thuận, Phường Thanh Điền, Tỉnh Tây Ninh, Việt Nam</div>
            </a>
          </div>
          {/* <a
            href="#"
          ...
          <nav className="hidden md:flex items-center gap-8">
            {navLinks.map((link) => (
              <button
                ...
                className={`text-sm tracking-widest uppercase font-sans font-medium transition-colors duration-300 hover:text-gold cursor-pointer ${scrolled ? 'text-foreground/80' : 'text-white/90'
                  }`}
              >
                {link.label}
              </button>
